...
def meta_partition(graph: dgl.DGLGraph, target_node_type: str, num_parts: int, out_path: str, 
    ...
    print("Partition Time: {:.4f}s".format(time.time()-start_time))
    start_time = time.time()
    ...
    print(f"Peak memory usage: {get_peak_mem():.2f} GB")
    print("Saving Time: {:.4f}s".format(time.time()-start_time))
...
if __name__=="__main__":
    ...
    args.out_dir = os.path.join(args.out_dir, args.dataset + f"_{args.num_parts}parts" + f"_depth_{args.depth}")
    os.makedirs(args.out_dir, exist_ok=True)
    start = time.time()
    g, num_classes, target_node_type, list_of_metapaths, reverse_edge_type_prefix = load_dataset(args.dataset, args.root)
