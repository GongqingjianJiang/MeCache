...
def gpu_capacity():
    max_device_mem = th.cuda.mem_get_info()[1]/1024/1024
    max_allocated_mem = th.cuda.max_memory_allocated()/1024/1024
...
def load_partition(part_id: int, graph_name: str, local_group: dist.ProcessGroup) -> dgl.DGLGraph:
    ...
    if args.local_rank == 0:
        ...
        print("path", os.path.join(args.part_dir, f"part{part_id}/graph.dgl"))
...
def run(args, device, data, local_group: dist.ProcessGroup, mirror_group: dist.ProcessGroup):
    ...
    model = get_model(args.model, g, args.predict_category, args.embed_dim, args.num_hidden, n_classes, 
                      args.num_layers, dist=False, process_group=mirror_group)
    ...
    for epoch in range(args.num_epochs):
        ...
        print(f"Part {args.rank}, gpu cache read hit rate: {embed_layer_module.cache_hit_rate}")
        if args.dgl_sparse and emb_optimizer is not None:
