    ...
    def __init__(
        ...
        for ntype in ntypes_w_feat:
            ...
            if cache_method != 'none':
                ...
                try:
                    cache_nodes = np.load(f"cache/{label}/{dataset}_{cache_method}_{args.model}/part{self._machine_id}/{ntype}.npy")
                    # cache_nodes=cache_nodes[:int(0.9*len(cache_nodes))]
        ...
        if dgl_sparse_emb:
            ...
            for ntype in sorted(ntypes_wo_feat):
                ...
                if cache_method != 'none' and part_size > 0 and label != 'dgl':
                    ...
                    # local_cache_nodes=local_cache_nodes[:int(0.9*len(local_cache_nodes))]
        ...
        for ntype in node_ids:
            ...
            if ntype in self.ntypes_w_feat:
                if ntype in self._gpu_caches:
                    ...
                    self._cache_hit_rate[ntype].append(gpu_cache.cache_read_hit_rate)
                ...
                if self.node_embeds[ntype].gpu_cache is not None:
                    cache_hit_rate = self.node_embeds[ntype].gpu_cache.cache_read_hit_rate
