...
def load_igb_full(dataset:str='small', root:str='dataset', use19: bool =True, load_feat: bool = True):
    ...
    if "medium" in dataset or "large" in dataset:
    ...
    if 'large' in dataset:
        paper_node_labels = torch.from_numpy(np.memmap(os.path.join(root, dataset, 'processed', 'paper', label_name), dtype='float32', mode='r',  shape=(n_nodes)))
    else:
...
def load_stackexchange(root: str = 'dataset', load_feat: bool = True):
    # 定义边文件的路径和对应的关系
    edges_files = {
        ("Badges", "Badges-Us/gf3/home/jgqj/test_code/hydro/log/drgnn_rgat_mag240m_none_64_4_newrun.log-UserId", "Users"): "Comments:Comments-UserId:Users_edges.npy",
...
def load_dataset(dataset: str, root: str = 'dataset', load_feat: bool = True, complete_missing_feats: bool = False):
    ...
    elif dataset == 'igb-full-small':   # 晚点改回来，试试L2pca和普通pca在2k标签下效果如何；
    ...
    elif dataset == 'igb-full-large':   # need 800g dram to loadfeat.
        return load_igb_full('hete_large', root, use19=True, load_feat=False)
    else:
