...
def copy_graph_to_shared_mem(g: dgl.DGLHeteroGraph, graph_name: str, rank: int,  local_group: dist.ProcessGroup):
    ...
    start = time.time()
    # transfer mask to uint8
    for ntype in g.ntypes:
        for key in g.nodes[ntype].data.keys():
            if g.nodes[ntype].data[key].dtype==torch.bool:
                print(f'transfering {ntype} {key} from bool to uint!')
                g.nodes[ntype].data[key]=g.nodes[ntype].data[key].to(torch.uint8)
