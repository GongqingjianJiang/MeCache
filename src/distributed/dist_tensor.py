...
from dgl.distributed.dist_context import is_initialized
from dgl.distributed.kvstore import get_kvstore
from dgl.distributed.role import get_role
from dgl.distributed.rpc import get_group_id
...
class DistTensor:
    ...
    def __init__(
        ...
        if self._name not in exist_names:
            print(f"device {torch.distributed.get_rank()} spawn:",self._name)
            self._owner = True
            self.kvstore.init_data(
                self._name, shape, dtype, part_policy, init_func, is_gdata
            )
        else:
            print(f"device {torch.distributed.get_rank()} get:",self._name)
            self._owner = False
    ...
    def __getitem__(self, idx):
        if self._gpu_cache is not None:
            # print(f"__getitem__: rank {torch.distributed.get_rank()}, gpu device {self._gpu_cache.device}, buffer id {id(self._gpu_cache.cache_buffer)}")
    ...
    def __setitem__(self, idx, val):
        if self._gpu_cache is not None:
            # print(f"__setitem__: rank {torch.distributed.get_rank()}, gpu device {self._gpu_cache.device}, buffer id {id(self._gpu_cache.cache_buffer)}")
            device = self._gpu_cache.device
            ...
            if self._gpu_cache.write_through:
                ...
                if len(uncached_idx) > 0:
                    # print("not empty!",self._tensor_name,self._gpu_cache.device,uncached_idx)
                    start = time.time()
                    uncached_idx = uncached_idx.to('cpu')
                    uncached_val = uncached_val.to('cpu')
                    # uncached_idx = uncached_idx.to('cpu',non_blocking=True)
                    # uncached_val = uncached_val.to('cpu',non_blocking=True)
                    self._h2d_d2h_time = time.time() - start
